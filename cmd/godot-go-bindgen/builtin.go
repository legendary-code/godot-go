...
func emitBuiltinClass(api *API, buildConfig string, bc *BuiltinClass, cfg *genConfig) error {
	...
	if err != nil {
		_ = os.WriteFile(filepath.Join(cfg.Dir, strings.ToLower(bc.Name)+".gen.go.broken"), buf.Bytes(), 0o644)
		return fmt.Errorf("gofmt: %w", err)
	}

	outPath := filepath.Join(cfg.Dir, strings.ToLower(bc.Name)+".gen.go")
